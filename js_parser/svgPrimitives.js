...
const extractStylePropsFromTheme = theme => {
...
const extractStyleAttrsFromTheme = theme => {
    return SvgStyleFieldsMap.map(
        item => (theme[item.from] ? `${item.to}="${theme[item.from]}"` : null)
    )
        .filter(i => i)
...
const getRectangle = (x, y, w, h, theme) => {
    return `<rect x="${x}" y="${y}"
            width="${w}" height="${h}"
            style="${extractStylePropsFromTheme(theme)}" />`;
};

const getLine = (x1, y1, x2, y2, theme) => {
    return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"
                style="${extractStylePropsFromTheme(theme)}" />`;
};

const getCircle = (x, y, r, theme) => {
    return `<circle cx="${x}" cy="${y}" r="${r}"
        style="${extractStylePropsFromTheme(theme)}" />`;
};

const getText = (x, y, theme, text) => {
    return `<text x="${x}" y="${y}"
...
const getClosedPath = (points, theme) => {
...
const getCurvedPath = (points, theme) => {
    const pointStr = points
        .map((point, i) => {
            if (!i) return `M${point.x}, ${point.y}`;
...
module.exports = {
    extractStylePropsFromTheme,
    extractStyleAttrsFromTheme,
    getRhombus,
    getRoundedRectangle,
    getRectangle,
    getLine,
