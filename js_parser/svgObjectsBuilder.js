const { complexTraversal } = require('shared/utils/traversalWithTreeLevelsPointer');
const { SVGBase } = require('./SVGBase');
const { createShapeForNode, createRootCircle, createConnectionArrow } = require('./shapesFactory');
const { TOKEN_TYPES, TOKEN_KEYS, ARROW_TYPE } = require('shared/constants');

const buildSVGObjectsTree = (flowTree, styleTheme) => {
    const svg = SVGBase();
...
const buildShapeStructures = (flowTree, styleTheme) => {
...
const buildConnections = (shapesTree, styleTheme) => {
    const connections = [],
        pushArrow = config => {
...
module.exports = {
    buildSVGObjectsTree,
    buildShapeStructures,
    addExtraSpacingBeforeShape,
    buildConnections,
    isNoArrow,
